...
export default function PlanetAcademy({ planetId }: PlanetAcademyProps) {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#040712] text-white">
      ...
      <div className="pointer-events-none fixed inset-0 opacity-[0.045] [background-image:linear-gradient(rgba(255,255,255,.7)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,.7)_1px,transparent_1px)] [background-size:38px_38px]" />
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-7xl flex-col px-4 pb-10 pt-5 sm:px-7 lg:px-10">
        <header className="flex flex-wrap items-center justify-between gap-4 border-b border-white/10 pb-5">
          <Link to="/" className="group inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.035] px-3.5 py-2 font-mono text-[9px] uppercase tracking-[0.15em] text-slate-400 transition hover:border-white/25 hover:text-white">
            <ArrowLeft className="h-3.5 w-3.5 transition group-hover:-translate-x-0.5" /> Solar map
          </Link>
        ...
        <section className="grid gap-7 py-8 lg:grid-cols-[1.2fr_.8fr] lg:items-end lg:py-12">
          <div>
            ...
            <h1 className="font-display text-5xl font-semibold tracking-[-0.05em] sm:text-7xl">{academy.name}</h1>
          ...
          <div className="rounded-[1.75rem] border border-white/10 bg-white/[0.035] p-5 shadow-2xl backdrop-blur-xl">
        ...
        <nav className="grid gap-2 border-y border-white/10 py-3 sm:grid-cols-3">
          {academy.missions.map((item, index) => {
            ...
            return (
              <button key={item.id} onClick={() => setActiveMission(index)} className={`group flex min-w-0 items-center gap-3 rounded-2xl border px-3 py-3 text-left transition ${current ? "bg-white/[0.08]" : "border-transparent hover:bg-white/[0.035]"}`} style={{ borderColor: current ? `${academy.color}80` : undefined, boxShadow: current ? `0 0 22px ${academy.glow}` : undefined }}>
        ...
        <AnimatePresence mode="wait">
          <motion.section key={mission.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.22 }} className="grid flex-1 gap-4 py-6 lg:min-h-[560px] lg:grid-cols-12">
            <article className="flex flex-col rounded-[1.7rem] border border-white/10 bg-white/[0.035] p-5 backdrop-blur-xl lg:col-span-5">
              ...
              <div className="mt-5 space-y-5 text-[13px] leading-6 text-slate-300">
                ...
                <div className="rounded-2xl border border-white/[0.09] bg-black/20 p-4"><p className="mb-1.5 font-mono text-[8px] font-bold uppercase tracking-[0.17em]" style={{ color: academy.color }}>Under the hood</p><p>{mission.technicalTheory}</p></div>
            ...
            <div className="flex flex-col gap-4 lg:col-span-4">
              ...
              {mission.terminalCommand ? (
                <article className="flex min-h-[220px] flex-1 flex-col overflow-hidden rounded-[1.7rem] border border-white/10 bg-[#03060b] shadow-[inset_0_1px_0_rgba(255,255,255,.05)]">
                  <div className="flex items-center justify-between border-b border-white/10 bg-white/[0.035] px-4 py-3"><div className="flex items-center gap-2 font-mono text-[9px] font-bold uppercase tracking-[0.14em] text-slate-300"><TerminalSquare className="h-4 w-4" style={{ color: academy.color }} /> Preview shell</div><span className="font-mono text-[7px] uppercase tracking-widest text-amber-300">simulation</span></div>
                  ...
                  <button onClick={() => { setTerminalInput(mission.terminalCommand ?? ""); }} className="border-t border-white/5 px-4 py-2 text-left font-mono text-[8px] text-slate-500 transition hover:bg-white/[0.025] hover:text-slate-300">Suggested: <span style={{ color: academy.color }}>{mission.terminalCommand}</span></button>
                </article>
              ) : (
                <article className="flex flex-1 flex-col justify-between rounded-[1.7rem] border border-white/10 bg-white/[0.025] p-5"><div><LockKeyhole className="h-5 w-5" style={{ color: academy.color }} /><p className="mt-3 font-mono text-[9px] uppercase tracking-[0.17em] text-slate-500">Practical lab unlocks in mission 03</p><p className="mt-2 text-[12px] leading-6 text-slate-400">Use the signal board to build intuition first. The final checkpoint turns that understanding into a safe, simulated command workflow.</p></div><div className="font-mono text-[9px] uppercase tracking-widest text-slate-600">No terminal initialized</div></article>
              )}
            </div>

            <article className="flex flex-col rounded-[1.7rem] border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl lg:col-span-3">
              ...
              <AnimatePresence>{hasAnswer && <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className={`mt-4 rounded-xl border p-3 text-[10px] leading-5 ${isCorrect ? "border-emerald-400/20 bg-emerald-400/[0.08] text-emerald-100" : "border-amber-300/20 bg-amber-300/[0.08] text-amber-100"}`}><p className="mb-1 font-mono text-[8px] font-bold uppercase tracking-widest">{isCorrect ? "Correct reasoning" : "Review the reasoning"}</p>{mission.explanation}</motion.div>}</AnimatePresence>
              <div className="mt-auto pt-5"><button onClick={completeMission} disabled={!canLockCheckpoint} className="flex w-full items-center justify-center gap-2 rounded-xl px-3 py-3 font-mono text-[9px] font-bold uppercase tracking-wider text-slate-950 transition enabled:hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-35" style={{ backgroundColor: academy.color }}>{completed.has(activeMission) ? <><CheckCircle2 className="h-3.5 w-3.5" /> Checkpoint locked</> : <>{mission.terminalCommand && !canLockCheckpoint ? "Run mission command first" : "Lock checkpoint"}<ChevronRight className="h-3.5 w-3.5" /></>}</button></div>
            </article>
          </motion.section>
        </AnimatePresence>

        <footer className="mt-auto flex flex-wrap items-center justify-between gap-3 border-t border-white/10 pt-5">
          <button onClick={() => setActiveMission((current) => Math.max(0, current - 1))} disabled={activeMission === 0} className="inline-flex items-center gap-2 rounded-full border border-white/10 px-4 py-2.5 font-mono text-[9px] uppercase tracking-wider text-slate-400 transition hover:border-white/25 hover:text-white disabled:cursor-not-allowed disabled:opacity-30"><ArrowLeft className="h-3.5 w-3.5" /> Previous mission</button>
          <p className="font-mono text-[8px] uppercase tracking-[0.16em] text-slate-600">Preview curriculum · content is designed for extension</p>
          {activeMission < academy.missions.length - 1 ? <button onClick={() => setActiveMission((current) => current + 1)} className="inline-flex items-center gap-2 rounded-full px-4 py-2.5 font-mono text-[9px] font-bold uppercase tracking-wider text-slate-950 transition hover:brightness-110" style={{ backgroundColor: academy.color }}>Next mission <ArrowRight className="h-3.5 w-3.5" /></button> : <button onClick={() => { setCompleted(new Set()); savePlanetAcademyCompletion(planetId, 0); setActiveMission(0); setTerminalPassed({}); setAnswers({}); setTerminalLogs(["COSMOSX PREVIEW SHELL // simulation provider online"]); }} className="inline-flex items-center gap-2 rounded-full border border-white/10 px-4 py-2.5 font-mono text-[9px] uppercase tracking-wider text-slate-400 transition hover:border-white/25 hover:text-white"><RotateCcw className="h-3.5 w-3.5" /> Reset academy</button>}
        </footer>
      </div>
    </main>
  );
